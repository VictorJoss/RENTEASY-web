        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {loading ? (
            ...
            properties.map((prop, idx) => (
              <Link href={`/propiedad/${prop.id}`} key={prop.id}>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: idx * 0.1 }}
                  viewport={{ once: true }}
                  className="rounded-xl overflow-hidden shadow-lg bg-white/70 border border-white/20 hover:shadow-xl transition-all flex flex-col h-full"
                >
